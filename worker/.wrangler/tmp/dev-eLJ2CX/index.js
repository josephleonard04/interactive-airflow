...
// .wrangler/tmp/bundle-WBhMUM/strip-cf-connecting-ip-header.js
...
var init_error = __esm({
  "node_modules/@anthropic-ai/sdk/core/error.mjs"() {
    ...
    APIError = class extends AnthropicError {
      ...
      static generate(status, errorResponse, message, headers) {
        ...
        if (status === 409) {
          return new ConflictError(status, error3, message, headers, type);
        }
        if (status === 422) {
        ...
        return new APIError(status, error3, message, headers, type);
      }
    };
...
var init_node_browser = __esm({
  "node_modules/@anthropic-ai/sdk/tools/agent-toolset/node.browser.mjs"() {
    ...
    __name(BashSession, "BashSession");
...
// .wrangler/tmp/bundle-WBhMUM/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
// .wrangler/tmp/bundle-WBhMUM/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
__name(WorkloadIdentityError, "WorkloadIdentityError");
...
var loadConfigWithSource = /* @__PURE__ */ __name(async (profile3) => {
  var _a2, _b;
...
var getCredentialsPath = /* @__PURE__ */ __name(async (config2, profile3) => {
...
function userOAuthProvider(config2) {
  return async (opts) => {
    ...
    let raw;
    try {
      raw = await fs.promises.readFile(config2.credentialsPath, "utf-8");
    } catch (err) {
...
__name(BidirectionalPageCursor, "BidirectionalPageCursor");
...
__name(DeploymentRuns, "DeploymentRuns");
...
__name(Deployments, "Deployments");
...
__name(UserProfiles, "UserProfiles");
...
__name(SessionToolRunner, "SessionToolRunner");
...
/* @__PURE__ */ __name(async function _SessionToolRunner_routeToolEvent2(ev) {
  const permission2 = ev.evaluated_permission;
  const verdict = permission2 === "deny" ? "deny" : __classPrivateFieldGet(this, _SessionToolRunner_confirmationVerdicts, "f").get(ev.id);
  if (verdict === void 0) {
    if (permission2 === void 0 || permission2 === "allow") {
...
__name(EnvironmentWorker, "EnvironmentWorker");
...
__name(Environments, "Environments");
...
__name(MemoryVersions, "MemoryVersions");
...
__name(MemoryStores, "MemoryStores");
...
__name(JSONLDecoder, "JSONLDecoder");
...
__name(BetaMessageStream, "BetaMessageStream");
...
__name(BetaToolRunner, "BetaToolRunner");
...
function referencedToolName(ref2) {
  switch (ref2.type) {
    case "tool_reference":
      return ref2.name;
    default:
...
var BaseAnthropic = class {
  ...
  async fetchWithTimeout(url, init, ms, controller, requestOptions, logCtx) {
    ...
    if (signal) {
      ...
      registerRequestSignalCleanup(controller, signal, abort2);
    }
...
__name(BaseAnthropic, "BaseAnthropic");
...
var SCALARS = ["temperature", "contaminant", "draft"];
var DIRECTIONS = ["low", "high"];
var SCHEMA = {
  type: "object",
  properties: {
    objectives: {
...
var SYSTEM = `You translate a non-expert's everyday comfort wish into a small fixed set of physical objectives an airflow simulator can score.
...
var src_default = {
  async fetch(request, env2) {
    ...
    if (url.pathname === "/api/health") {
      return json({ goalLlm: Boolean(env2.ANTHROPIC_API_KEY), model: MODEL }, 200, cors);
    }
    if (url.pathname !== "/api/parse-goal")
    ...
    const rooms = Array.isArray(body.rooms) ? body.rooms.filter((r) => r && typeof r.id === "string").slice(0, MAX_ROOMS).map((r) => ({ id: r.id, name: String(r.name ?? ""), type: String(r.type ?? "") })) : [];
    if (!text)
    ...
    let raw;
    try {
      const client = new Anthropic({ apiKey: env2.ANTHROPIC_API_KEY });
...
${text}` }
    ...
    const objectives = (parsed.objectives ?? []).filter(
      (o) => SCALARS.includes(o.scalar) && DIRECTIONS.includes(o.direction)
    ).map((o) => ({
      scalar: o.scalar,
