...
%% ---------------------------------------------------------------------------
%%  CAMERA-READY AUTHOR BLOCK — commented out because CHI review is anonymous.
%%
%%  TODO(Joseph): confirm your own affiliation. I have used Virginia Tech,
%%  inferred from the study contact address josephl04@vt.edu. If the work
%%  should be credited to the University of Tokyo visit instead, change it.
%% ---------------------------------------------------------------------------
%
...
Flow is solved by an incompressible Euler solver on a staggered (MAC) grid with
buoyancy, running entirely in the browser. The home is voxelized into solids,
jets, inlets and outlets; temperature, contaminant and drying are then
transported along the converged flow by a geodesic pass. Exterior glazing chills
the air against it so that cold downdrafts form where they physically would.

One modelling detail matters enough to state, because the task set turns on it.
A cooling jet is roughly 8--10\,K below room temperature and therefore denser
than the air around it, so a louvre aimed down is working with gravity and one
aimed up is working against it. Aimed down, the jet reaches the floor and
spreads as a gravity current into corners that direct throw never reaches; aimed
up, it stalls within a metre and falls back over the unit that threw it --- the
short-circuiting that is the reason no cooling louvre is aimed at a ceiling. An
earlier symmetric treatment of tilt made these two indistinguishable and the
apartment task had two contradictory answers.
...
discriminates (Table~\ref{tab:cascade}). Because the final rung is the fidelity
the goal checks themselves use, a suggestion's printed numbers and its checkmarks
cannot disagree (D4).

\begin{table}[t]
  \caption{The three rungs of the cascade. Each is a coarser discretization of
    the same Euler solve.}
  \label{tab:cascade}
  \small
  \begin{tabular}{lrrrl}
    \toprule
    Rung & Cells & Iters & Steps & Role \\
    \midrule
    Screen & 1{,}200 & 4 & 8  & ranks all candidates, 300-eval budget \\
    Mid    & 1{,}800 & 6 & 12 & re-ranks the shortlist \\
    Final  & 4{,}200 & 8 & 22 & re-scores finalists at goal fidelity \\
    \bottomrule
  \end{tabular}
\end{table}
...
The solver is a coarse real-time prototype of roughly 18{,}000 cells. It is
calibrated for the four study tasks and has not been validated against physical
measurement; the claims here are about a consistent interactive model, not about
predictive accuracy for a real building.
